...
export default function Index({ 
  ...
  return (
    <>
      ...
      <Header 
        title="Notes" 
        subtitle={`${currentNotes.length} ${currentNotes.length === 1 ? 'note' : 'notes'} total`}
        showBackButton={true}
        backHref="/dashboard"
      />
              ...
              {hasNotes ? (
                <>
                  ...
                  {currentNotes.filter(note => note.pinned).length > 0 && (
                    <div className="mb-8">
                      <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                        ...
                        <h2 className="text-xl sm:text-2xl font-bold text-white">Pinned Notes</h2>
                        <span className="text-xs sm:text-sm text-[#98989D] bg-[#3A3A3C] px-2 py-1 rounded-full">
                          {currentNotes.filter(note => note.pinned).length}
                        </span>
                      </div>
                      <div className={viewType === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6' : 'space-y-4'}>
                        <AnimatePresence>
                  ...
                  {currentNotes.filter(note => !note.pinned).length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-[#3A3A3C] rounded-full flex items-center justify-center">
                          <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#98989D]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="text-center py-12 sm:py-16">
                  <div className="max-w-md mx-auto px-4">
                    ...
                    <h3 className="text-lg sm:text-xl font-semibold text-white mb-2">No notes yet</h3>
                    <p className="text-sm sm:text-base text-[#98989D] mb-4 sm:mb-6">Get started by creating your first note.</p>
                    <button
                      onClick={() => setIsFormVisible(true)}
