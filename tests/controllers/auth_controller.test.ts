...
test.group('Auth Controller', (group) => {
  ...
  test('GET /api/v1/auth/me - should return current user when authenticated', async ({ client }) => {
    // Create and login user
    const user = await User.create({
      fullName: 'Test User',
  ...
  test('POST /api/v1/auth/forgot-password - should send password reset email', async ({ client }) => {
    await User.create({
      fullName: 'Test User',
  ...
  test('POST /api/v1/auth/reset-password - should reset password with valid token', async ({ client }) => {
    await User.create({
      fullName: 'Test User',
